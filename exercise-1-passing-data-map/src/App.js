...
class chartList extends Component{
  render()
  {
    const {myList,bookList,peepsList}=this.props
    return(
      <div>
        {myList.map(a=><li key="name">{peepsList[a.peepsID].name+"'s likes a "+bookList[a.bookLikedID].name+" book"}</li>)}
      </div>
    )
  }
}
class App extends Component {
  ...
  render() {
    ...
    return (
      ...
      <ul>
      <chartList myList={chart} bookList={books} peepsList={peeps}/>
       </ul>
